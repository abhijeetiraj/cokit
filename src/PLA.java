...
public class PLA extends DemoModule
{
    int windowCount = 0;
    Icon icon;
    ...
    public PLA(CoKit cokit)
    {
        super(cokit, "PLA", "toolbar/PLA.gif");
    ...
    public JInternalFrame createInternalFrame(Integer layer, int width, int height, boolean state)
    {
        ...
        jif.setTitle("Field Programmable PLA");


        JPanel p = new JPanel();
        p.setLayout(new BorderLayout());

        p.add(new PlaDesignPanel(equation, stringExpression, varChoice), BorderLayout.CENTER);

        JScrollPane scroller = new JScrollPane();
        scroller.getViewport().add(p);

        JSplitPane splitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, true, new gateDocPanel(equation, truthTable, varChoice,
    ...
    class ShowFrameAction extends AbstractAction
    {
        ...
        public ShowFrameAction(PLA demo)
        {
            this.demo = demo;
